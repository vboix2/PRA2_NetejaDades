...
La segona pregunta se centra en les variables qualitatives (*Sex*, *Cabin*, *Embarked* i *Title*). En aquesta anàlisi utilitzarem el contrast d'hipòtesis per comparar la variació de la mitjana de supervivents per a les diferents categories de cada variable. Per tant, per a la variable *Sex* que només conté dos valors (*male* i *female*) realitzarem una prova t de Student per comparar aquestes dues poblacions; mentre que per a la resta de variables (*Cabin*, *Embarked* i *Title*) farem un test ANOVA per comparar les diferents poblacions de cada categoria.

Finalment, a la tercera pregunta treballarem amb el conjunt *train* per crear un model de regressió logística a partir de totes les variables, tant les numèriques com les categòriques. Aquest model l'utilitzarem per efectuar prediccions sobre el conjunt de *test*.
...
A la segona anàlisi ens preguntem si les diferents categories de les variables qualitatives (*Sex*, *Cabin*, *Embarked* i *Title*) influeixen en la seva probabilitat de sobreviure. Atès a què la variable *Survived* és qualitativa, emprarem el test Chi quadrat amb la hipotesi nul·la que diu que les variables examinades no siguin independents.
A L'execució podem veure com el p-valor és inferior al 0.05 a tots els casos pel que rebutjam la hipòtesi nul·la. Amb tota probabilitat hi ha una forta relació entre *Survived* i les columnes analitzades

```{r}
#obtenim la p-valor per a les columnes següents

col_names =c( 'Survived','Sex', 'Cabin', 'Embarked','Title')


for (col in col_names){
  df_chi = xtabs(train[, c("Survived", col)])
  print(df_chi)
  print(chisq.test(df_chi))
}
