...
```{r}
# Extraiem el títol de l'atribut Name, situat entre ',' i '.'.
train$Titol <- sub(".*\\,\\s([A-z\\s]*)\\.\\s.*", "\\1", train$Name)
test$Titol <- sub(".*\\,\\s([A-z\\s]*)\\.\\s.*", "\\1", test$Name)
...
train <- merge(train, tickets, by="Ticket")
test <- merge(test, tickets, by="Ticket")
