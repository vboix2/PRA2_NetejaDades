...
**Age**
Per imputat el valor buit de l'edat (*Age*) farem servir el mètode d'imputació basada en els k-veïns més pròxims (KNN). El que fa l'algorisme knn-imputation és identificar les k-observacions més properes segons la distància euclidiana i assigna la mitjana ponderada a l'atribut amb valor NA.

```{r}
# Imputació de valors amb VIM
train$Age <- kNN(train)$Age 
test$Age <- kNN(test)$Age 
```
Tornem a comprovar els valors nuls
```{r}
# Nombre de valors NA per atribut
sapply(train, function(x) sum(is.na(x)))
sapply(test, function(x) sum(is.na(x)))
...
**Cabin**
Al cas de l'atribut *Cabin* substituirem els valors buits pel marcador 'UNKNOWN', afegim la nova categoria al factor i eliminem els que no usem.
```{r}
clean_cabin<- function(factor_var){
  levels(factor_var$Cabin) <- c(levels(factor_var$Cabin), 'UNKNOWN')
  factor_var[factor_var$Cabin==''|is.na(factor_var$Cabin), 'Cabin'] <- 'UNKNOWN'
  droplevels(factor_var$Cabin) 
}
