...
En primer lloc, analitzarem la normalitat de la distribució de l'atribut *Age*, primer de manera gràfica i després amb el test de Shapiro-Wilk, com la mostra es relativament gran (|T| > 50), verificarem els resultats amb el test d'Anderson-Darlin.
...
```
### 4.2.2. Anàlisi de les distribucions de *Age* i *Fare*
...
### 4.2.3. Anàlisi d'homogeneitat de la variancia

Per tal de discriminar si la variància de les variables numèriques *Age* i *Fare*, és constant als grups de les persones que hi sobreviuen, respecte a les que no ho fan, es realitza l'anàlisi de l'homogeneïtat de la variància, com cap de les dues de les variables té una distribució normal es realitza el test Fligner-Killeen, que permet comparar les medianes de les variàncies en lloc de les mitjanes amb mostres de diferent grandària.
En aquest test la hipòtesi nul·la contempla que les variàncies entre els grups són iguals.
**Age**
```{r}
...
fligner.test(x = list(a,b))
```
Com el p-valor resultat és pràcticament 0, podem rebutjar la hipòtesi nul·la. Això ens indica que la variància de la variable *Fare* és significativament diferent en ambdós grups.
Si observem els dos boxplots:

```{r}
#convertirm Survived a factor per a la correcta visualització i el guardem a una nova columna

train$Survived.factor <- as.factor(train$Survived)
ggplot(data = train, aes(x = Survived.factor, y = Fare, colour = train$Survived.factor)) +
  geom_boxplot() +
  geom_point() +
  theme_bw() +
...
```{r}
# Calculem la probabilitat de supervivència dels passatgers del conjunt train per a obtenir la precissió del model
prob_Survived <- predict(regLog, train, type="response")
y_train <- cbind(train, prob_Survived)
head(y_train,5)
```
Per a obtenir el llindar més adequat de probabilitat observem la corva de ROC. 
```{R}
library(ROCR)
library(pROC)
plot.roc(y_train$Survived, y_train$prob_Survived,  main="Confidence interval of a threshold", percent=TRUE,  ci=TRUE, of="thresholds", # compute AUC (of threshold)  
...
y_train$Survived_pred<-factor(y_train$Survived_pred)
CrossTable(y_train$Survived,y_train$Survived_pred)
```

### 4.3.4 Generació de la predicció
...
solution$Survived_pred<-factor(solution$Survived_pred)
head(solution,5)
```
...
Les variables més significatives per al model són Sex, Title, Cabin, Group. Agruparem per aquestes variables per a obtenir un dataframe preparat per un tree Map.

```{r}
library("reghelper")
beta(regLog)

#contem per Sex, Title, Cabin, Group i Fare.
df_for_treemap <- count(solution, c("Survived_pred","Sex", "Title", "Cabin"))

```
```{r}
library(treemap)
treemap(df_for_treemap,index = c("Survived_pred","Sex", "Title", "Cabin"),
        vSize ="freq",
        vColor = "freq",
        type="value",
        palette="RdYlGn",
        range=c(0,200),
        mapping=c(0,200),
        title = "Sales Treemap For categories",
...
```{r}
library(ggforce)
names(solution)[names(solution) == "Sex"] <- "DSex"
names(solution)[names(solution) == "Title"] <- "CTitle"
names(solution)[names(solution) == "Cabin"] <- "BCab"
names(solution)[names(solution) == "Survived_pred"] <- "ASurv"

data <- reshape2::melt(solution)
data <- gather_set_data(data, c("DSex", "CTitle", "BCab", "ASurv"))

ggplot(data, aes(x, id = id, split = y, value = value)) +
  geom_parallel_sets(aes(fill = ASurv), alpha = 0.3, axis.width = 0.1) +
