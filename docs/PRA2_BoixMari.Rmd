...
A continuació modificarem alguns atributs per tal de reduir el nombre de categories i obtenir-ne informació que pugui resultat interessant per a la fase d'anàlisi.

**Name**

A l'atribut *Name* i volem extreure el títol de tractament que si diferència un estatus que podria influir en la supervivencia de l'individu.
Als casos a on el tractament no està inclòs al nom, si són homes, li assignem "Mr" i si són dones (Miss o Mrs) li assignarem el tractament fent servir KNN. 

````{r}
library(stringr)

train$Title <- str_extract(string = train$Name,pattern = "(Mr|Miss|Mrs)\\.")
test$Title <- str_extract(string = test$Name,pattern = "(Mr|Miss|Mrs)\\.")
train$Title[is.na(train$Title) & train$Sex =='male'] <- 'Mr'
test$Title[is.na(test$Title) & test$Sex =='male'] <- 'Mr'
train[train$Sex=='female',]$Title <- kNN(train[train$Sex=='female',], k=3)$Title 
test[test$Sex=='female',]$Title <- kNN(test[test$Sex=='female',], k=3)$Title 
...
Si mostrem el percentatge de supervivents per tipus de cabina veiem que resulta força alt per a determinades lletres i molt baix per als passatgers que no tenien cap cabina assignada.

```{r}
# Percentatge de supervivents per tipus de cabina
aggregate(Survived~Cabin, data=train, mean)
```
...
**Age**
Analitzarem la normalitat de la distribució de l'atribut *Age*, primerament de manera gràfica i després amb el test d'Anderson-Darlings (a tots els atributs).
